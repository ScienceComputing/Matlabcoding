# Continue: code linear algebra in MATLAB

- [A function that uses triple-nested loops to compute the matrix-matrix multiplication](MatMatMult.m)
- [A function that computes the matrix-matrix multiplication by one column at a time](Gemm_unb_var1.m)
- [A function that computes the matrix-matrix multiplication by one row at a time](Gemm_unb_var2.m)
- [A function that computes the matrix-matrix multiplication with rank-1 updates](Gemm_unb_var3.m)
- [A function that performs the Gaussian elimination](GaussianElimination.m)
- [A function that performs the forward substitution](ForwardSubstitution.m)
- [A function that performs the LU factorization](LU_unb_var5.m)
- [A function that performs the forward substitution, solving Lz = b when L is unit lower triangular](Ltrsv_unb_var1.m)
- [A function that performs the backward substitution, solving Ux = b](Utrsv_unb_var1.m)
- [A function that performs the LU factorization, forward substitution, and backward substitution](Solve.m)
- [An alternative function that performs the forward substitution, solving Lz = b when L is unit lower triangular](Ltrsv_unb_var2.m)
